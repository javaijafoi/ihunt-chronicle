...
interface CharacterSheetProps {
  ...
  // Selfie Actions
  onAddSelfie?: () => void; // Simplified to void for now, will likely open modal
  onDeleteSelfie?: (id: string) => void;
  onViewSelfie?: (selfie: Selfie) => void; // Optional view handler
}
...
export function CharacterSheet({
  ...
  const [activeTab, setActiveTab] = useState<'sheet' | 'selfies' | 'bio'>('sheet');
  ...
  */
  const sheetBody = (
    <div className="relative w-full max-w-4xl space-y-6">
      ...
      {/* Refactored Tabs Layout */}
      <Tabs defaultValue="sheet" className="w-full" onValueChange={(v) => setActiveTab(v as any)}>
        <TabsList className="grid w-full grid-cols-3 mb-4 bg-muted/20">
          <TabsTrigger value="sheet" className="font-ui font-medium uppercase tracking-wider text-xs">Ficha</TabsTrigger>
          <TabsTrigger value="selfies" className="font-ui font-medium uppercase tracking-wider text-xs flex items-center gap-2">
            Selfies <span className="bg-primary/20 text-primary px-1.5 rounded-full text-[10px]">{character.selfies?.length || 0}</span>
          </TabsTrigger>
          <TabsTrigger value="bio" className="font-ui font-medium uppercase tracking-wider text-xs">Bio & Notas</TabsTrigger>
        </TabsList>

        {/* --- TAB: SHEET (FICHA) --- */}
        <TabsContent value="sheet" className="space-y-6 animate-in fade-in-50 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Consequences & Notes (Partial - Notes moved to Bio tab) */}
        ...
        {/* --- TAB: SELFIES --- */}
        <TabsContent value="selfies" className="h-[600px] animate-in fade-in-50 duration-300">
          <SelfieMural
            selfies={character.selfies || []}
            readOnly={readOnly}
            onAddSelfie={onAddSelfie}
            onDeleteSelfie={onDeleteSelfie}
            onViewSelfie={onViewSelfie}
          />
        </TabsContent>

        {/* --- TAB: BIO & NOTES --- */}
        <TabsContent value="bio" className="animate-in fade-in-50 duration-300">
          <div className="glass-panel p-4 space-y-6">
            <div>
              <h4 className="text-xs font-bold uppercase text-muted-foreground mb-4 flex items-center gap-2">
                <Brain className="w-4 h-4" /> Anotações & Detalhes
              </h4>
              {readOnly ? (
                <div className="text-sm text-foreground/80 whitespace-pre-wrap p-4 bg-muted/20 rounded-lg border border-white/5 min-h-[200px] leading-relaxed">
                  {character.notes || 'Sem anotações.'}
                </div>
              ) : (
                <textarea
                  className="w-full min-h-[300px] bg-background/50 border border-border rounded-lg p-4 text-sm focus:outline-none focus:border-primary resize-y font-ui leading-relaxed"
                  placeholder="Escreva sobre a biografia, contatos, pistas ou teorias do personagem..."
